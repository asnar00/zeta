...
# VMEntity: superclass for various types of blocks
class VMEntity:
    pass
    
# instruction block : just a list of instructions
class VMBlock(VMEntity):
    def __init__(self):
        self.instructions = []
        self.vars = {}              # str => VMVar
...
# VMForLoop is a loop that repeats some VMEntity
class VMForLoop(VMEntity):
    def __init__(self, loop_var: VMVar, start: VMEntity, inc: VMEntity, test: VMEntity, body: VMEntity):
...
class VMProgram(VMEntity):
    def __init__(self):
        self.blocks : List[VMEntity] = []
